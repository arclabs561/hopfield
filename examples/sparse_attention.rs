//! Dense vs sparse memory support.
//!
...
fn main() {
    ...
    println!(
        "LSE support > 1e-6: {}; sparsemax support > 1e-12: {}",
        support(&lse_weights, 1e-6),
        support(&sparse_weights, 1e-12)
    );
    println!(
        "LSE entropy: {:.4}; sparsemax entropy: {:.4}",
        entropy(&lse_weights),
        entropy(&sparse_weights)
    );
    println!("top LSE weights: {:?}", top_weights(&lse_weights, 6));
    println!(
        "top sparsemax weights: {:?}",
        top_weights(&sparse_weights, 6)
    );
    println!(
        "distance(query, LSE retrieval): {:.6}; distance(query, sparse retrieval): {:.6}",
        l2(&query, &lse),
        l2(&query, &sparse)
    );
}
